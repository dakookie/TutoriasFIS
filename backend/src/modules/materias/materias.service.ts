...
export class MateriasService {
  ...
  async findByNombre(nombre: string): Promise<MateriaDocument | null> {
    return this.materiaModel.findOne({
  ...
  async cambiarEstado(id: string, activo: boolean): Promise<MateriaDocument> {
    if (!Types.ObjectId.isValid(id)) {
      throw new BadRequestException('ID de materia inválido');
    }
  ...
  async contar(): Promise<number> {
    return this.materiaModel.countDocuments({ activo: true });
  }
}
